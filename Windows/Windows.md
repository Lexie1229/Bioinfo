# [Setting-up scripts for Windows 10](https://github.com/wang-q/windows/blob/master/README.md)
...
* DISM（Deployment Image Servicing and Management）：部署映像服务和管理工具(DISM.exe)
    * 命令行工具，位于Windows安装的c:\windows\system32文件夹中
...
* profile：配置文件
* Windows PowerShell 和 PowerShell 的PowerShell语言存在不同：
    * 在Windows和非Windows平台上PowerShell cmdlet的可用性和行为存在差异
    * .NET Framework 和 .NET Core 之间差异引起的差异
...
* winget-pkgs：Windows程序包管理器，WinGet命令行实用工具可从命令行安装应用程序和其他程序包
    ...
    * winget install：安装给定的程序包
        ...
        * winget install -v(--version)：指定安装的版本，否则安装最高版本的应用程序
...
* programming 编程
    * JDK：Java语言的软件开发工具包，包含Java的运行环境（JVM）、Java基础类库和Java工具
